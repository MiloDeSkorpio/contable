import React from 'react';
//import data
import { footer } from '../data';
//import components


const Testimonials = () => {
  //destructure testimonials
const { form } = footer
  return (
    <section className='bg-blue-50 section' id='contacto'>
      <div className='container mx-auto'>
        {/** title **/} 
        <h2 
          className='title mb-10 lg:mb-20 text-center max-w-[920px] mx-auto'
          data-aos='fade-up'
          data-aos-delay='200'
        >Contactanos</h2>
        <div
            data-aos='fade-up'
            data-aos-offset='200'
            data-aos-delay='900'
          >
              {/** form **/}
              <form className='bg-blue-100 max-w-[600px] rounded-lg mb-[10px] '>
              <h2 className='text-4xl font-semibold mb-5 text-center'>Ingresa tus Datos</h2>
                <div className='h-[62px] p-[7px] flex flex-col justify-center items-center'>
                  <label className='font-semibold pr-4'>Nombre</label>
                  <input 
                    type='text' 
                    className='w-96 h-full p-6 border-none rounded-lg outline-none placeholder:text-dark '
                    placeholder={form.placeholder}
                  />
                </div>
                <div className='h-[62px] p-[7px] flex flex-col justify-center items-center'>
                  <label className='font-semibold pr-4'>Correo</label>
                  <input 
                    type='text' 
                    className='w-96 h-full p-6 border-none rounded-lg outline-none placeholder:text-dark '
                    placeholder={form.placeholder}
                  />
                </div>
                <div className='h-[62px] p-[7px] flex flex-col justify-center items-center'>
                  <label className='font-semibold pr-4'>Asunto</label>
                  <input 
                    type='text' 
                    className='w-96 h-full p-6 border-none rounded-lg outline-none placeholder:text-dark '
                    placeholder={form.placeholder}
                  />
                </div>
                <div className='h-[62px] p-[7px] flex flex-col justify-center items-center'>
                  <label className='font-semibold pr-4'>Mensaje</label>
                  <input 
                    type='text' 
                    className='w-96 h-full p-6 border-none rounded-lg outline-none placeholder:text-dark '
                    placeholder={form.placeholder}
                  />
                </div>
                <button className='btn btn-sm bg-blue-500 hover:bg-blue-600 w-[102px] text-white'>{form.btnText}</button>
              </form>
              <span className='text-sm text-light'>{form.smallText}</span>
          </div>
      </div>

    </section>
  )
};

export default Testimonials;
